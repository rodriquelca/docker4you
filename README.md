...
```
$ git clone git@github.com/rodriquelca/docker4you.git && cd docker4you
```

**The rest of the commands are to be run from the root of the project.**

Copy `.env.example` to `.env`, both at the root of the project and in `src/processmaker`:
...
$ docker compose exec processmaker php artisan
$ docker compose exec processmaker composer
```

For example, you could run Laravel's default database migrations to confirm the MySQL connection is working:

```
$ docker compose exec processmaker php artisan migrate
```
